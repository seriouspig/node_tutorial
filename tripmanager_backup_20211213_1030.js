...
const https = require('https')  // ADDED HTTPS REQUEST XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
const http = require('http')    // ADDED HTTPS REQUEST XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
// var getIatas = (callback) => callback(iatas)

var getIatas = (callback) => {
    let flightAll
    wowboxGetRequest(wowboxApiConfig.endpoint.flightInfo)
        .then((response) => flightAll = response)
        .then(() => iatas[0] = flightAll.route.origin)
        .then(() => iatas[1] = flightAll.route.destination)
        .then(() => callback(iatas))
}
        ...
        getIatas((newIatas) => {
            if (newIatas) {
                ...
                if (iatas[0].length == 3) {
                    ...
                    return mainLoop();
                } else {   // Converting Icaos to Iatas if more than 4 chars, don't need this
...
var runMainLoop = () => {
    ...
    connectionPool.getConnection((err, conn) => {
        ...
        connection = conn;
        sqlSteps.checkOverride(false).then((result) => { // changed true to false - TEMP
            closeConnection(connection);
            if (result === 1) {
...
const tripManager = {
    ...
    api: {
        ...
        //         origin: JSON.parse(result.getBody().toString()).route.origin,
        //         destination: JSON.parse(result.getBody().toString()).route.destination
        //     })
...
var initialise = () => {
    ...
    connectionPool.getConnection((err, conn) => {
        if (err) {
            ...
            sqlSteps.resetOverride(false).then((result) => { // changed true to false - TEMP
                console.log(result);
                runMainLoop();
            }).catch((err) => {
                console.log(err);
