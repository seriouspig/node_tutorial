...
const http = require('http')    // ADDED HTTP REQUIRE XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
var airbusGetRequest = (endpoint) => {              // OLD GetRequest for Airbus - left it for now XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
var wowboxGetRequest = (endpoint) => new Promise((resolve, reject) => {
    console.log('Trying to get Flight Data')
    http.get(wowboxApiConfig.getURL() + endpoint, (resp) => {
        let data = ''
...
var getTail = (callback) => callback("AB4215")
...
function getIatas (callback) {
    wowboxGetRequest(wowboxApiConfig.endpoint.flightInfo)
        .then((response) => {
            var origin = response.route.origin
            var destination = response.route.destination
            if (!origin) { origin = ""}
            if (!destination) {destination = ""}
            iatas = [origin, destination]})
        .then(() => callback(iatas))
}
...
var tail;
var tripId;
var iatas = ["",""]; // Maybe should defign as array with empty strings /////////////////////////////////
var stopId;
        ...
        connection.query(
            ...
            (err, result) => {
                ...
                if (result && result.length > 0) {
                    ...
                    tripId = result[0].id;
                    console.log("XXXXXXX Found the trip result:" + tripId)

                    return res();
                } else {
                    console.log("XXXXXXX No tripId found")
                    return rej(0);
                }
            });
    }),
...
var mainLoop = () => {
        ...
        [sqlSteps.setAllStopsInactiveForTrip, sqlSteps.setStopActive].reduce((promiseChain, currentTask) => {
            ...
            if (!rejectionRetryTimer) {
                retryTimer = setTimeout(() => {
                    runMainLoop();
                }, config.simulated ? 10000 : 10000);  // Checking every 2 mins (120000) otherwise every 10 seconds ////////////////////////////////////////////////////////////
            }
        });
    };
    ...
    if (shouldCheckTail) {
        ...
        getIatas((newIatas) => {
            if (newIatas) {
                ...
                if (iatas[0].length == 3) {
                    ...
                    return mainLoop();
                } else if (iatas[0].length == 4) {   // Converting Icaos to Iatas if more than 4 chars, don't need this
                ...
                if (retryTimer) {
                    console.log("XXXXXXXXXXXXXXX COULD NOT GET IATAS") // TEMP /////////////////////////
                    clearTimeout(retryTimer);
                    retryTimer = undefined;
                }
        ...
        connectionPool.getConnection((err, conn) => {
            ...
            sqlSteps.selectStop.standard(false).then((result) => { // changed true to false - TEMP
                if (result === 0) {
...
var runMainLoop = () => {
    ...
    connectionPool.getConnection((err, conn) => {
        ...
        connection = conn;
        sqlSteps.checkOverride(false).then((result) => { // changed true to false //////////////////////////////////
            closeConnection(connection);
            if (result === 1) {
...
const tripManager = {
    ...
    api: {
        ...
        // getFlightPair: (req, res) => {               // KEPT FOR FOR AIRBUS ////////////////////////////////
        ...
        getFlightPair: (req, res) => {
            wowboxGetRequest(wowboxApiConfig.endpoint.flightInfo)                
                .then( (result) => flightPair =  {
                        origin: (result.route.origin),
                        destination: (result.route.destination)
                    })
                .then(() => console.log(iatas))
                .then(() => { return res.json(flightPair) })
...
var initialise = () => {
    ...
    connectionPool.getConnection((err, conn) => {
        if (err) {
            ...
            sqlSteps.resetOverride(false).then((result) => { // changed true to false //////////////////////////////////
                console.log(result);
                runMainLoop();
            }).catch((err) => {
                console.log(err);
